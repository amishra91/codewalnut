...
interface TimerModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: Omit<Timer, 'id' | 'createdAt'>) => void;
...
export const TimerModal: React.FC<TimerModalProps> = ({
  ...
  const [hours, setHours] = useState(
    Math.floor((initialData.duration || 0) / 3600)
  );
  const [minutes, setMinutes] = useState(
    Math.floor(((initialData.duration || 0) % 3600) / 60)
  );
  const [seconds, setSeconds] = useState((initialData.duration || 0) % 60);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setHours(Math.floor((initialData.duration || 0) / 3600));
      setMinutes(Math.floor(((initialData.duration || 0) % 3600) / 60));
      setSeconds((initialData.duration || 0) % 60);
      setTouched({
        title: false,
        hours: false,
        minutes: false,
